...
async function collectBusyBlocks(
  ...
  // Fetch ERP activities for the date
  try {
    ...
    console.warn('[book] ERP busy fetch failed:', String(e))
  }
...
export async function POST(
  ...
  const { rows: tokenRows } = await pool.query(
    `SELECT encode(gen_random_bytes(32), 'hex') AS cancel_token`
  )
  const cancelToken: string = tokenRows[0].cancel_token
