...
export async function executeBooking(params: BookingParams): Promise<BookingResult> {
  ...
  // --- 4. Insert booking record with status 'pending' BEFORE creating external events ---
  // This ensures the cancel token always exists in the DB even if event creation crashes.
  const { rows: pendingRows } = await pool.query(
    `INSERT INTO bookings
       (account_id, template_id, share_link_id, booker_email, booked_date, booked_time,
        duration_minutes, field_values, cancel_token, status, created_erp_ids, created_outlook_id, created_google_id)
  ...
  // --- 5. Create activities in configured targets ---
  ...
  try {
    ...
    // Unexpected top-level error during event creation — mark booking as failed
    console.error('[book] Event creation failed unexpectedly:', String(eventCreationError))
    await pool.query(
  ...
  // --- 6. Update booking record to 'confirmed' with the created external IDs ---
  const { rows: bookingRows } = await pool.query(
    `UPDATE bookings
     SET status = 'confirmed', created_erp_ids = $1, created_outlook_id = $2, created_google_id = $3
     WHERE id = $4
     RETURNING *`,
    [
  ...
  // --- 7. Send notification emails (only after confirmed) ---
